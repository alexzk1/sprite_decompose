...
void WorkArea::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    auto tmpi = itemAt(position, QTransform());
    if (tmpi)
    {
        QGraphicsObject *obj = tmpi->toGraphicsObject();
...
void WorkArea::toggledMagnet(bool checked)
{
    m_animation->setMagnetism(checked);
}
...
void WorkArea::changeTolerance(int tolerance)
{
    m_cutAuto_tolerance = tolerance;
}
void WorkArea::changeConstraintHeight(bool enabled)
{
    m_constraintHeight = enabled;
}
void WorkArea::changeConstraintWidth(bool enabled)
{
    m_constraintWidth = enabled;
}
